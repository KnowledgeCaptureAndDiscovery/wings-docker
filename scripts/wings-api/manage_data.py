...
parser.add_argument("-x", "--delete", help="Delete operation", action="store_true")
parser.add_argument("-dt", "--data_type", help="Data type")
parser.add_argument("-pt", "--parent_type", help="Data type's Parent (for creating new data type)")
parser.add_argument("-d", "--data_id", help="Data id")
parser.add_argument("-m", "--save_metadata", help="Save metadata for data (filename with json metadata)")
parser.add_argument("-pr", "--add_properties", help="Add properties for data type (filename with json properties)")
parser.add_argument("-l", "--data_location", help="Set data location")
parser.add_argument("-up", "--upload_data", help="Upload data from file for a data type")
parser.add_argument("-g", "--get", help="Get information (use either with data_id, data_type or none)", action="store_true")
