...
class ManageData(UserOperation):
	...
	def new_data_type(self, dtype, parent):
		...
		self.session.post( self.get_request_url() + 'data/newDataType', postdata )
	...
	def add_type_properties(self, dtype, properties):
		...
		self.session.post( self.get_request_url() + 'data/saveDataTypeJSON', postdata )
		
	def add_data_for_type(self, dataid, dtype):
		dtype = self.get_type_id(dtype)
		dataid = self.get_data_id(dataid)
		postdata = { 'data_id': dataid, 'data_type': dtype }
		self.session.post( self.get_request_url() + 'data/addDataForType', postdata )
		
	def del_data_type(self, dtype):
		dtype = self.get_type_id(dtype)
		postdata = { 'data_type': json.dumps([dtype]), 'del_children': 'true' }
		self.session.post( self.get_request_url() + 'data/delDataTypes', postdata )
	...
	def get_all_items(self):
		resp = self.session.get( self.get_request_url() + 'data/getDataHierarchyJSON' )
		return resp.json()
	...
	def save_metadata(self, dataid, metadata):
		...
		for key in metadata:
			if(metadata[key]):
				pvals.append({'name': self.dcdom + key, 'value': metadata[key]})
		postdata = { 'propvals_json': json.dumps(pvals), 'data_id': self.get_data_id(dataid) }
		self.session.post( self.get_request_url() + 'data/saveDataJSON', postdata )

	def set_data_location(self, dataid, location):
		postdata = { 'data_id': self.get_data_id(dataid), 'location': location }
		self.session.post( self.get_request_url() + 'data/setDataLocation', postdata )
